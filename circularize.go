...
func main() {
	...
	fmt.Println("\n# \"scarely 2 minutes\" non-impulsive velocity change")
	Mflow := 700.0    // kg/s
	M := 193652.0     // kg
	...
	for loopCounter := 1; t <= 875242.500000; t, loopCounter = t+dt, loopCounter+1 {
	...
	V0 = math.Sqrt(Vx*Vx + Vy*Vy)
	V = V0
	fmt.Printf("# begin non-impulsive circularization maneuver\n")
	fmt.Printf("# t %.03f, V0 %.03f, Mass %.03f\n", t, V0, M)
	...
	for ; M >= 25652. && (V0-V) < 2721.340; t += dt {
		...
		fmt.Printf("%.03f\t%.03f\t%.03f\t%.03f\t%.03f\t%.03f\n", t, Vx, Vy, X, Y, M)
